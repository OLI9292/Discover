...
url_semantic = "https://allennlp.s3.amazonaws.com/models/srl-model-2018.05.25.tar.gz"
predictor_semantic = Predictor.from_path(url_semantic)
...
def index_texts():
    ...
    job = q.enqueue_call(
        func=index_text, args=(filename, index, is_rob), timeout=6000, result_ttl=5000
    )
...
@app.route("/mine-properties")
def mine_properties():
    try:
        sentence = request.args.get("sentence")
        predictions = predictor_semantic.predict(sentence=sentence)

        results = []

        for prediction in predictions["verbs"]:
            description = prediction["description"]
            arg1 = find_between("[ARG1: ", "]", description)
            arg2 = find_between("[ARG2: ", "]", description)
            verb = find_between("[V: ", "]", description)
            if (arg1 != "") & (arg2 != ""):
                results.append({"word": arg1, "properties": arg2.split(" ")})
...
def discover_images():
    try:
        words = [
            s.strip() for s in request.args.get("words").encode("utf-8").split(",")
        ]
...
def upload_images():
    try:
        ...
        job = q.enqueue_call(
            func=upload_images_to_s3, args=(request.json), timeout=6000, result_ttl=5000
        )
